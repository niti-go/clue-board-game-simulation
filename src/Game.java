...
public class Game {
    Game() {
        ...
        do {
            playAgain = "";
            System.out.println("\nWelcome to Rainbow Mansion. Your host, Mr. Boe, has met an untimely end — "
                    + "he's the victim of foul play. To win this game, you must determine the answer to these three questions: "
                    + "Who done it? Where? And with what weapon?\n\nThree cards containing the "
                    + "answers to these questions have been kept in a confidential case file. "
                    + "The remaining cards have been distributed among you and two other players. "
                    + "Any card that is in your hand or another player's hand can't be "
                    + "in the case file — which means none of those cards were involved in the crime! "
                    + "Your task is to figure out which cards are in the Case File by narrowing down "
                    + "the suspects, weapons, and rooms.\n\nYou, Comp 1, and Comp 2 will take turns "
                    + "interrogating each other. Each time, you will interrogate another player "
                    + "about 3 cards, and they will show you one of those cards, if they have any. "
                    + "Then, you can eliminate that card from your list of suspects, since it "
                    + "isn't in the case file. Your notes will be updated automatically each time "
                    + "you take a turn, and you can reference them in the Notes.txt file. "
                    + "You wil have to press the \"enter\" key to continue to each person's turn. "
                    + "\n\nEvery 3 turns, you will be asked if you would like to make a final "
                    + "accusation. This is when you feel confident that you know all the "
                    + "information about the crime, and are ready to accuse the person, weapon, "
                    + "and room that were involved in the Case File. You only get to make the "
                    + "final accusation once; if you are correct, you win, but if you are incorrect,"
                    + " you lose. Be sure you make a final accusation as quickly as possible before "
                    + "any other player does! Now let's play!");

            input = new Scanner(System.in);
            String isReady = " ";
            // Declaring and initializing the card decks of people, weapons, and place
            ...
            // Creating the card deck of people cards
            ...
            // Creating the card deck of weapon cards
            ...
            // Creating the card deck of location cards
            ...
            // Preparing lists for the USER's notes
            ArrayList<String> personList = new ArrayList<String>();
            ArrayList<String> weaponList = new ArrayList<String>();
            ArrayList<String> placeList = new ArrayList<String>();

            for (String person : personCards) {
                personList.add(person);
            }
            for (String weapon : weaponCards) {
                weaponList.add(weapon);
            }
            for (String place : placeCards) {
                placeList.add(place);
            }

            // Preparing lists for the COMP 1's notes
            ...
            for (String person : personCards) {
                comp1personList.add(person);
            }
            for (String weapon : weaponCards) {
                comp1weaponList.add(weapon);
            }
            for (String place : placeCards) {
                comp1placeList.add(place);
            }

            // Preparing lists for the COMP 2's notes
            ...
            for (String person : personCards) {
                comp2personList.add(person);
            }
            for (String weapon : weaponCards) {
                comp2weaponList.add(weapon);
            }
            for (String place : placeCards) {
                comp2placeList.add(place);
            }
            ...
            // Write information to notes files, and remove the cards that each individual
            // has from their
            ...
            do {
                ...
                input.nextLine(); // The player should press "enter" each time to continue to the next person's
                                  // turn
                cardFound = comp1Turn(comp1Cards, comp2Cards, userCards, comp1personList, comp1weaponList,
                        comp1placeList);
                ...
                writeNotes("Comp1Notes.txt", comp1personList, comp1weaponList, comp1placeList, comp1Cards); // edit the
                                                                                                            // notes
                                                                                                            // file
                ...
                cardFound = comp2Turn(comp2Cards, userCards, comp1Cards, comp2personList, comp2weaponList,
                        comp2placeList);
                ...
                writeNotes("Comp2Notes.txt", comp2personList, comp2weaponList, comp2placeList, comp2Cards); // edit the
                                                                                                            // notes
                                                                                                            // file
            ...
            if (isReady.equals("Y") || isReady.equals("y")) {
                ...
                if (accusation.equals(caseFile)) {
                    ...
                    System.out.print("\n\nSorry. You Lose D:\n The correct cards that were involved in the crime were "
                            + caseFile);
                }
            }
            ...
            // Otherwise, if Comp 1 was ready to make an accusation, let them make it
                ...
                System.out.print("\nComp 1 has made a final accusation... Comp 1 guessed that " + caseFile
            ...
            // Otherwise, if Comp 2 was ready to make an accusation, let them make it
                ...
                System.out.println("\nComp 2 has made a final accusation... Comp 2 guessed that the case file contains "
                        + caseFile);
    ...
    public static void writeNotes(String fileName, ArrayList<String> personList, ArrayList<String> weaponList,
            ArrayList<String> placeList, ArrayList<String> playerCards) {
    ...
    // USER'S TURN METHOD
    public static String usersTurn(ArrayList<String> playerCards, ArrayList<String> comp1Cards,
            ArrayList<String> comp2Cards, ArrayList<String> personList, ArrayList<String> weaponList,
        ...
        do {
            ...
            System.out.print("\n\nYou are interrogating Comp 1 about " + interrogation
                    + ". Make sure you have not made spelling errors. Are you sure you want to move on? (Y or N): ");
        } while (input.nextLine().equals("N"));

        ArrayList<String> hasCards = new ArrayList<String>();
        // Using the Compare to methods made in ThreeCards.java to determine which cards
        // Comp 1 has of the interrogation
        ...
        if (interrogation.hasNoneOfThose(comp1Cards) == true) // If Comp 1 didn't have any of the cards of the
                                                              // interrogation, proceed to ask Comp 2
        {
            System.out
    ...
    public static String comp1Turn(ArrayList<String> playerCards, ArrayList<String> comp2Cards,
            ArrayList<String> userCards, ArrayList<String> personList, ArrayList<String> weaponList,
            ArrayList<String> placeList) {
        Scanner input = new Scanner(System.in);
        // These will be the three cards that Comp 1 chooses for the interrogation
        ThreeCards interrogation = new ThreeCards();
        ...
        // Randomizing three cards from Comp 1's list of remaining suspects to be part
        // of the interrogation
        // There is a 30% chance that Comp 1 will ask about a card they already have, to
        // throw off the other players sometimes
        if (personList.size() > 1 && rand1 > 0.3) {
            ...
            } while (card1 != "Col. Mustard" && card1 != "Mrs. Peacock" && card1 != "Prof. Plum"
                    && card1 != "Miss Scarlet" && card1 != "Mrs. White" && card1 != "Mr. Green");
        }
        ...
        if (weaponList.size() > 1 && rand2 > 0.3) {
            ...
            } while (card2 != "Wrench" && card2 != "Lead Pipe" && card2 != "Pistol" && card2 != "Dagger"
                    && card2 != "Candlestick" && card2 != "Rope");
        }
        if (placeList.size() > 1 && rand3 > 0.3) {
            ...
            } while (card3 != "Living Room" && card3 != "Garage" && card3 != "Courtyard" && card3 != "Office"
                    && card3 != "Bathroom" && card3 != "Game Room" && card3 != "Kitchen" && card3 != "Dining Room"
                    && card3 != "Bedroom");
        }
        ...
        // Carry out the interrogation. Store the cards that Comp 2 has in the ArrayList
        // hasCards
        ArrayList<String> hasCards = new ArrayList<String>();
        // Using the Compare to methods made in ThreeCards.java to determine which cards
        // Comp 2 has
        ...
        int counter = 0; // Counts how many of the cards of the interrogation that the User has, if the
                         // User is asked.
        if (interrogation.hasNoneOfThose(comp2Cards) == true) {
            ...
            if (interrogation.hasNoneOfThose(userCards) == true) {
                ...
                // Now the player may have just figured out 1 or more of the cards in the Case
                // File.
                boolean cardIsTherePerson = false;
                boolean cardIsThereWeapon = false;
                boolean cardIsTherePlace = false;
                ...
                // Check if the person they interrogated about is in their suspect list. If it
                // is, then since none of the
                // other players had it in their hand, that must be the card in the Case File

                // Otherwise, if the the person is not in their suspect list, then they actually
                // had that card in their hand
                // So they weren't really reavealed any new information (Think about asking
                // another player if they had Col. Mustard, but
                // you have Col. Mustard in your hand, so obviously they would not have Col.
                // Mustard)
                for (int x = 0; x < personList.size(); x++) {
                    if (personList.get(x).equals(interrogation.getPersonCard())) {
                        cardIsTherePerson = true;
                ...
                // If they figured out the person, then remove every other person from their
                // suspect list except for that person.
                if (cardIsTherePerson == true) {
                    for (int x = 0; x < personList.size(); x++) {
                        if (!personList.get(x).equals(interrogation.getPersonCard())) {
                            personList.remove(x);
                ...
                // If they figured out the weapon, then remove every other weapon from their
                // suspect list except for that weapon.
                if (cardIsThereWeapon == true) {
                    for (int x = 0; x < weaponList.size(); x++) {
                        if (!weaponList.get(x).equals(interrogation.getWeaponCard())) {
                            weaponList.remove(x);
                ...
                // If they figured out the room, then remove every other room from their suspect
                // list except for that room.
                if (cardIsTherePlace == true) {
                    for (int x = 0; x < placeList.size(); x++) {
                        if (!placeList.get(x).equals(interrogation.getPlaceCard())) {
                            placeList.remove(x);
        ...
        if (hasCards.size() > 0 && counter > 1) {
            ...
            cardShow = hasCards.get((int) Math.random() * hasCards.size()); // Randomize the card from hasCards that
                                                                            // Comp 1 will be shown
    ...
    public static String comp2Turn(ArrayList<String> playerCards, ArrayList<String> userCards,
            ArrayList<String> comp1Cards, ArrayList<String> personList, ArrayList<String> weaponList,
            ArrayList<String> placeList) {
        Scanner input = new Scanner(System.in);
        ...
        ArrayList<String> hasCards = new ArrayList<String>();
        // Using the Compare to methods made in ThreeCards.java to have the Computer
        // print out one card if they have it
        int counter = 0;
        if (interrogation.comparePerson(userCards) == true) {
    ...
    public static void removeFromNotes(ArrayList<String> personList, ArrayList<String> weaponList,
            ArrayList<String> placeList, ArrayList<String> playerCards) {
        for (int i = 0; i < personList.size(); i++) {
